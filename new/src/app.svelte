<script lang="ts">
  ...
  import QuestionMark from './modules/icons/questionMark.svelte'
  ...
  $: calendarDays = Array.from({ length:
...
<main class="h-full overflow-y-hidden sm:overflow-auto space-y-2">
  <div class="flex justify-between px-2 sm:px-4 py-2 items-center">
    <div>
      <h1 class="text-2xl sm:text-3xl text-blue-500">{`<thai-tech-calendar />`}</h1>
      <!-- <p class="text-gray-950 pt-1 text-sm sm:text-base">รวม อัพเดท Tech event, Tech Meetup ในไทยไว้ในที่เดียว</p> -->
    </div>
    <button class="w-10 h-10 bg-gray-200 flex items-center justify-center rounded-xl border-b-4 border-b-gray-300 shrink-0 ml-2">
