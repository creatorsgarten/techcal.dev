...
{#if item}
  <div class="text-xl flex flex-col justify-center">
    <div>
      <div class="mb-2">
        <strong class="bg-sky-400 text-black px-2 py-1 text-sm rounded"
          >Tech event</strong
        >
      </div>
      <h1
        id="capture-title"
        class="text-5xl font-bold mb-4 [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical] overflow-hidden leading-tight bg-clip-text bg-linear-to-r from-sky-300 to-pink-300 text-transparent"
      >
        {item.summary}
      </h1>
      {#if item.location}
